...
| `/limits set max-ai-credits VALUE` | Set a soft maximum for AI Credits allowed per response |
...
| `/mcp [config\|list\|show\|add\|edit\|delete\|disable\|enable\|auth\|reload\|search] [SERVER-NAME]` | Manage MCP server configuration |
...
| `/permissions [default\|assisted\|allow-all\|show]` | Switch between permission modes, or show the current mode |
| `/permissions reset` | Reset all in-memory tool and path approvals for the current session |
...
| `/skills add [--project] <FILE\|URL\|DIRECTORY>` | Add a skill from a file, URL, or directory |
| `/skills remove <NAME\|DIRECTORY>` | Remove a skill by name, or unregister a custom skill directory |
| `/skills reload` | Reload skills from all directories |
...
| `/code-review [low\|medium\|high\|xhigh\|max\|ultra] [--fix] [--comment] [pr#\|branch\|path]` | Skill command to review the current diff or a specified PR, branch, or path for correctness bugs and cleanup opportunities |
...
| `/design [brief]` | Skill command to draft UI mockups, screen flows, landing pages, or posters as artboards on a single canvas |
...
| `/rate-limit-options` | Show ways to keep working after reaching a claude.ai usage limit; type the command in full because it does not appear in the command menu |
...
| `/skill-doctor` | Show what each skill costs in context and how often it gets used |
...
| `/subtask <task>` | Spawn a forked background subagent that inherits the full conversation and reports its result back to the current session |
...
| `/workflow-authoring` | Skill command to load reference material for writing and editing dynamic workflow scripts |
